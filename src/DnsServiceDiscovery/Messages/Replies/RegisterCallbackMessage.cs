...
{
    class RegisterCallbackMessage : CallbackMessage<RegisterCallbackMessagePayload>
    {
        public RegisterCallbackMessage(ServiceMessageHeader header) : this(header, new RegisterCallbackMessagePayload())
        {
        }

        public RegisterCallbackMessage(CallbackMessageBaseValues baseValues, string instanceName, string serviceType, string domain)
            : this(new ServiceMessageHeader(OperationCode.RegisterServiceReply), new RegisterCallbackMessagePayload(baseValues, instanceName, serviceType, domain))
        {
        }

        public RegisterCallbackMessage(ServiceMessageHeader header, RegisterCallbackMessagePayload payload) : base(header, payload)
        {
            if (header.OperationCode != OperationCode.RegisterServiceReply)
                throw new ArgumentException($"Header contains incorrect operation code [{header.OperationCode}] for this class");
        }
    }

    internal class RegisterCallbackMessagePayload : CallbackMessagePayload
    {
        public string InstanceName { get; private set; } = string.Empty;
        public string ServiceType { get; private set; } = string.Empty;
        public string Domain { get; private set; } = string.Empty;

        public RegisterCallbackMessagePayload()
        {
        }

        public RegisterCallbackMessagePayload(CallbackMessageBaseValues baseValues, string instanceName, string serviceType, string domain) : base(baseValues)
        {
